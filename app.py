...
class AkilliOgrenciAsistani:
    ...
    def gemini_sohbet(self, mesaj: str) -> str:
        ...
        try:
            ...
            prompt = f"Sen bir akıllı öğrenci asistanısın. Elif adına konuş.  Samimi, eğlenceli,narsist, komik,ve motive edici bir dil kullan.Aynı zamanda biraz şimarık ol ve kullanıcıyla aranı samimi tutmak için laf sok.\n{context}\nKullanıcı: {mesaj}\nAsistan:"
            response = model.generate_content(prompt)
            return response.text
        except Exception as e:
